...
import { BabyLog, LogType, FeedLog, PumpLog } from '../types';
import { Milk, Baby, Clock, Droplets } from 'lucide-react';
...
export const Dashboard: React.FC<DashboardProps> = ({ logs }) => {
  ...
  const stats = useMemo(() => {
    ...
    // Last pump calculation（母親對上一次泵奶）
    const pumps = logs.filter(l => l.type === LogType.PUMP).sort((a, b) => new Date(b.timestamp).getTime() - new Date(a.timestamp).getTime());
    const lastPump = pumps.length > 0 ? (pumps[0] as PumpLog) : null;

    let lastPumpDisplay = "無";
    let lastPumpSub = "";
    if (lastPump) {
      lastPumpDisplay = `${lastPump.amountMl} ml`;
      const diffMs = new Date().getTime() - new Date(lastPump.timestamp).getTime();
      const diffHrs = Math.floor(diffMs / (1000 * 60 * 60));
      const diffMins = Math.floor((diffMs % (1000 * 60 * 60)) / (1000 * 60));
      lastPumpSub = `${diffHrs}小時 ${diffMins}分前`;
    }

    return { totalMilk, diaperCount, timeSinceFeed, lastPumpDisplay, lastPumpSub };
  }, [logs, today]);
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 gap-3">
        ...
        <div className="bg-purple-50 p-3 rounded-2xl flex flex-col items-center justify-center shadow-sm border border-purple-100">
          <Droplets className="w-6 h-6 text-purple-500 mb-1" />
          <span className="text-[10px] text-purple-400 font-bold uppercase">上次泵奶</span>
          <span className="text-lg font-black text-purple-700">{stats.lastPumpDisplay}</span>
          {stats.lastPumpSub && (
            <span className="text-[10px] text-purple-400 font-bold whitespace-nowrap">{stats.lastPumpSub}</span>
          )}
        </div>
      </div>
    </div>
